...
public class ChatRepository : IChatRepository
{
    ...
    public async Task<Chat?> GetByIdAsync(Guid chatId, CancellationToken cancellationToken)
    {
        try
        {
            var chat = await _dbContext.Chats.FirstAsync(u=>u.Id == chatId, cancellationToken);
            return chat;
        }
        catch (Exception ex)
        {
    ...
    {
        var foundChats = await _dbContext.Chats.AsNoTracking()
    ...
    {
        await _dbContext.Chats.Where(c => c.Id == id)
    ...
    {
        await _dbContext.Chats.Where(s => s.Id == id)
